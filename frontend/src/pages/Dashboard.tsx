...
import { getSalaryStatusColor, MonthlyAttendance, SalarySlip } from '@/types';
import api, { labMemberApi, adminApi, superAdminApi } from '@/services/api';
...
const LabMemberDashboard: React.FC = () => {
  ...
  // Use December 2025 where we have actual data
  // Database has attendance from Oct 2025 - Dec 2025
  const currentDate = new Date();
  const currentYear = 2025;  // Changed from currentDate.getFullYear()
  const currentMonth = 12;   // Changed from currentDate.getMonth() + 1 (December)
  ...
  const {
    ...
    queryKey: ['attendance', 'monthly', currentYear, currentMonth],
    queryFn: async () => {
      const response = await api.get(`/attendance/monthly/${currentYear}/${currentMonth}`);
      ...
      <Box
        sx={{
          ...
          background: `linear-gradient(135deg, 
            ${alpha(theme.palette.primary.light, 0.04)} 0%, 
            ${alpha(theme.palette.background.default, 1)} 50%,
            ${alpha(theme.palette.secondary.light, 0.04)} 100%)`,
        }}
      >
        <Box sx={{ position: "absolute", top: "5%", right: "10%", width: "400px", height: "400px", borderRadius: "50%", background: `radial-gradient(circle, ${alpha(theme.palette.primary.main, 0.12)} 0%, transparent 70%)`, filter: "blur(60px)", animation: "float1 18s ease-in-out infinite", "@keyframes float1": { "0%, 100%": { transform: "translate(0, 0) scale(1)" }, "50%": { transform: "translate(-80px, 60px) scale(1.1)" } } }} />
        <Box sx={{ position: "absolute", bottom: "10%", left: "15%", width: "350px", height: "350px", borderRadius: "50%", background: `radial-gradient(circle, ${alpha(theme.palette.secondary.main, 0.1)} 0%, transparent 70%)`, filter: "blur(50px)", animation: "float2 22s ease-in-out infinite", "@keyframes float2": { "0%, 100%": { transform: "translate(0, 0)" }, "50%": { transform: "translate(100px, -80px) scale(0.9)" } } }} />
        <Box sx={{ position: "absolute", top: "45%", left: "50%", width: "280px", height: "280px", borderRadius: "50%", background: `radial-gradient(circle, ${alpha(theme.palette.success.main, 0.08)} 0%, transparent 70%)`, filter: "blur(45px)", animation: "float3 20s ease-in-out infinite", "@keyframes float3": { "0%, 100%": { transform: "translate(-50%, -50%)" }, "50%": { transform: "translate(-30%, -70%) rotate(45deg)" } } }} />
      </Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 4, position: 'relative' }}>
          <Box>
            ...
            <Typography variant="h6" color="text.secondary" fontWeight={500}>
              Welcome back! Here's your attendance overview for {currentDate.toLocaleString('default', { month: 'long', year: 'numeric' })} </Typography>
          </Box>
          <Tooltip title="Refresh data">
            <IconButton onClick={handleRefresh} sx={{ bgcolor: alpha(theme.palette.primary.main, 0.1), boxShadow: 2, '&:hover': { bgcolor: alpha(theme.palette.primary.main, 0.2), transform: 'rotate(180deg)', transition: 'all 0.4s' } }}>
...
const AdminDashboard: React.FC = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const presentRecords = todayAttendance.filter((a: any) => a.status === 'PRESENT');
      const presentToday = presentRecords.length;
      const absentToday = activeMembers - presentToday;
      const attendanceRate = activeMembers > 0 ? Math.round((presentToday / activeMembers) * 100) : 0;

      // Extract present members for avatars
      const presentMembersList = presentRecords.map((a: any) => ({
        id: a.user?.employeeId || a.userId,
      ...
      return {
        ...
        absentToday,
        presentMembersList,
        onLeave: 0, // Can be calculated if you have leave data
        pendingApprovals: 0, // Can be fetched from leave applications
        attendanceRate,
        monthlyPayroll: 0, // Can be calculated from payroll data
        paidThisMonth: 0,
        averageSalary: 0,
      };
    }
  ...
  // Mock recent activities (Removed hardcoded data)
  const recentActivities: any[] = [];

  // Mock upcoming tasks (Removed hardcoded data)
  const upcomingTasks: any[] = [];

  // Mock attendance trend data (Last 7 days - placeholder)
  // Ideally this should be fetched from backend
  const attendanceTrend = [0, 0, 0, 0, 0, 0, 0];
  const weekDays = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];

  // Calculate trend
  // Calculate trend
  const lastWeekRate = 82;
  const trendPercentage = stats ? (stats.attendanceRate - lastWeekRate) : 0;
  ...
  return (
    <Box sx={{ position: "relative" }}>
      ...
      >
        <Box sx={{ position: "absolute", top: "10%", right: "5%", width: "420px", height: "420px", borderRadius: "50%", background: `radial-gradient(circle, ${alpha(theme.palette.primary.main, 0.12)} 0%, transparent 70%)`, filter: "blur(60px)", animation: "floatA1 20s ease-in-out infinite", "@keyframes floatA1": { "0%, 100%": { transform: "translate(0, 0) scale(1)" }, "50%": { transform: "translate(-100px, 70px) scale(1.15)" } } }} />
        <Box sx={{ position: "absolute", bottom: "15%", left: "10%", width: "380px", height: "380px", borderRadius: "50%", background: `radial-gradient(circle, ${alpha(theme.palette.success.main, 0.1)} 0%, transparent 70%)`, filter: "blur(50px)", animation: "floatA2 24s ease-in-out infinite", "@keyframes floatA2": { "0%, 100%": { transform: "translate(0, 0)" }, "50%": { transform: "translate(90px, -90px) scale(0.95)" } } }} />
        <Box sx={{ position: "absolute", top: "50%", left: "45%", width: "300px", height: "300px", borderRadius: "50%", background: `radial-gradient(circle, ${alpha(theme.palette.warning.main, 0.08)} 0%, transparent 70%)`, filter: "blur(45px)", animation: "floatA3 21s ease-in-out infinite", "@keyframes floatA3": { "0%, 100%": { transform: "translate(-50%, -50%)" }, "50%": { transform: "translate(-35%, -65%) scale(1.1)" } } }} />
      </Box>
          ...
          <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
            ...
            >
              <CardContent>
                ...
                <Typography variant="h3" fontWeight={700}>{stats?.totalMembers || 0}</Typography>
                <Typography variant="body2" sx={{ opacity: 0.9 }}>Total Lab Members</Typography>
                <Box sx={{ display: 'flex', alignItems: 'center', mt: 1, gap: 0.5 }}>
                  <CircleIcon sx={{ fontSize: 8, color: '#4ade80' }} />
                  <Typography variant="caption">{stats?.activeMembers || 0} active today</Typography>
                </Box>
              </CardContent>
            </Card>
          </Grid>
          ...
          <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
            ...
            >
              <CardContent>
                ...
                <Typography variant="h3" fontWeight={700}>{stats?.attendanceRate || 0}%</Typography>
          ...
          <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
            ...
            >
              <CardContent>
                ...
                <Typography variant="h3" fontWeight={700}>{stats?.pendingApprovals || 0}</Typography>
          ...
          <Grid size={{ xs: 12, sm: 6, lg: 3 }}>
            ...
            >
              <CardContent>
                ...
                <Typography variant="h3" fontWeight={700}>₹{((stats?.monthlyPayroll || 0) / 1000).toFixed(0)}K</Typography>
                <Typography variant="body2" sx={{ opacity: 0.9 }}>Monthly Payroll</Typography>
                <Typography variant="caption" sx={{ opacity: 0.7 }}>Avg: ₹{(stats?.averageSalary || 0).toLocaleString('en-IN')}/member</Typography>
              </CardContent>
            </Card>
          </Grid>
        </Grid>
                ...
                <Grid container spacing={2} sx={{ mb: 3 }}>
                  <Grid size={{ xs: 6, sm: 3 }}>
                    <Paper sx={{ p: 2, textAlign: 'center', bgcolor: alpha(theme.palette.success.main, 0.08), borderRadius: 2 }}>
                      ...
                      <Typography variant="h4" fontWeight={700} color="success.main">{stats?.activeMembers || 0}</Typography>
                  ...
                  <Grid size={{ xs: 6, sm: 3 }}>
                    <Paper sx={{ p: 2, textAlign: 'center', bgcolor: alpha(theme.palette.warning.main, 0.08), borderRadius: 2 }}>
                      ...
                      <Typography variant="h4" fontWeight={700} color="warning.main">{stats?.onLeave || 0}</Typography>
                  ...
                  <Grid size={{ xs: 6, sm: 3 }}>
                    <Paper sx={{ p: 2, textAlign: 'center', bgcolor: alpha(theme.palette.primary.main, 0.08), borderRadius: 2 }}>
                      ...
                      <Typography variant="h4" fontWeight={700} color="primary.main">{stats?.totalMembers || 0}</Typography>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Box>
                    ...
                    <AvatarGroup max={6} sx={{ justifyContent: 'flex-start' }}>
                      {(stats?.presentMembersList || []).slice(0, 5).map((member: any) => (
                        <Tooltip key={member.id} title={member.name}>
                          <Avatar
                            sx={{
                              bgcolor: `hsl(${((member.name?.charCodeAt(0) || 0) * 10) % 360}, 70%, 50%)`,
                              width: 36,
                              height: 36,
                            }}
                          >
                ...
                <Box sx={{ mb: 3 }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                    ...
                    <Typography variant="body2" fontWeight={600}>
                      {stats && stats.totalMembers > 0 ? Math.round((stats.paidThisMonth / stats.totalMembers) * 100) : 0}%
                    </Typography>
                  </Box>
                  <LinearProgress
                    variant="determinate"
                    value={stats && stats.totalMembers > 0 ? (stats.paidThisMonth / stats.totalMembers) * 100 : 0}
                    sx={{
                      height: 10,
                      borderRadius: 5,
                      bgcolor: 'grey.200',
                ...
                <Grid container spacing={2}>
                  <Grid size={{ xs: 4 }}>
                    <Box sx={{ textAlign: 'center', p: 2, borderRadius: 2, bgcolor: alpha(theme.palette.success.main, 0.08) }}>
                      ...
                      <Typography variant="h5" fontWeight={700} color="success.main">{stats?.paidThisMonth || 0}</Typography>
                  ...
                  <Grid size={{ xs: 4 }}>
                    <Box sx={{ textAlign: 'center', p: 2, borderRadius: 2, bgcolor: alpha(theme.palette.warning.main, 0.08) }}>
                      ...
                      <Typography variant="h5" fontWeight={700} color="warning.main">{stats?.pendingApprovals || 0}</Typography>
                  ...
                  <Grid size={{ xs: 4 }}>
                    <Box sx={{ textAlign: 'center', p: 2, borderRadius: 2, bgcolor: alpha(theme.palette.info.main, 0.08) }}>
                      ...
                      <Typography variant="h5" fontWeight={700} color="info.main">{(stats?.totalMembers || 0) - (stats?.paidThisMonth || 0) - (stats?.pendingApprovals || 0)}</Typography>
...
const SuperAdminDashboard: React.FC = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      // Fetch attendance data for today to calculate rate
      const today = new Date().toISOString().split('T')[0];
      const attendanceResponse = await api.get(`/attendance?date=${today}`);
      const todayAttendance = attendanceResponse.data || [];
      ...
      return {
        totalLabs: 1, // Defaulting to 1 as we don't have labs endpoint yet
        activeLabs: 1,
        totalMembers,
        activeMembers,
        superAdmins: users.filter((u: any) => u.role === 'SUPER_ADMIN').length,
        labAdmins: users.filter((u: any) => u.role === 'LAB_ADMIN').length,
        employees: users.filter((u: any) => u.role === 'EMPLOYEE' || u.role === 'LAB_MEMBER').length,
        attendanceRate,
        presentToday,
        totalPaidThisMonth: 0, // Placeholder until payroll API is available
        pendingSalaries: 0,
        pendingLeave: 0
      };
    }
